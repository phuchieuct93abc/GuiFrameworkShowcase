...
Calendar.getInstance();
		Optional<Path> folder = getWebContentFolder()
	...
	private static Optional<IProject> getWebContentFolder() {
		try {
			return Optional.ofNullable(ch.ivyteam.ivy.security.internal.SecurityManager.getSecurityManager()
					.executeAsSystem(() ->  Ivy.request().getProcessModelVersion().getProject()));
